...
chamfer mask, rotational dependency is usually ensured with increasing
mask size $m$ together with weight optimization. In this context and
for arbitrarily large image $N$, both Dijsktra-like and raster scan
approach have computational cost which is quadratic with respect to
...
\begin{equation}
  ...
  \end{equation}
defines metric spaces $(\Z^n, \R , d_{L_p})$ which are not
...
~\exists x\in P\quad A_kx=y_k.\footnote{$A_k$ being the $k^{th}$ row
  of $A$.}$ $f$ being the number of rows in $A$ and the number of
...
distances can be obtained from similar expression to (\ref{eq:1}):
\begin{equation}
...
In the following, we consider Voronoi map approach as defined in
\cite{BreuEtAl95}: Let us first define hyper-rectangular image $I_X:
...
computation. Indeed, distance comparison predicate can be implemented
error-free from integer number comparisons considering the $p$ power of
...
$e'_i>e_i$.
Let us first suppose that we do not have $e$ but we know the two wedges
